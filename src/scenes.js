...
Crafty.scene('Game', function(){
	...
	//如果地图比舞台小时，用于将地图居中
	//用舞台的宽减去地图的宽再除以2
	var paddingXToCenter = (Game.stageGrid.width * Game.stageGrid.cell.width - ground[0].length * Game.stageGrid.tile.width) / 2;
	//用舞台的高减去地图的高再除以2
	var paddingYToCenter = (Game.stageGrid.height * Game.stageGrid.cell.height - (ground.length * (Game.stageGrid.tile.height - Game.stageGrid.tile.floorHeight) + Game.stageGrid.tile.floorHeight)) / 2;
	
	//如果舞台的宽小于地图的宽时将paddingXToCenter设置为0
	if(paddingXToCenter < 0){
		paddingXToCenter = 0;
	}
	//如果舞台的高小于地图的高时将paddingYToCenter设置为0
	if(paddingYToCenter < 0){
		paddingYToCenter = 0;
	}
		...
		for(var col = 0; col < ground[row].length; col++){
			...
			if(tileComponent !== ' '){
				Crafty.e(tileComponent)
					.attr({
						x:col * Game.stageGrid.tile.width + paddingXToCenter,
						y:row * (Game.stageGrid.tile.height - Game.stageGrid.tile.floorHeight) + paddingYToCenter,
						z:row * (Game.stageGrid.tile.height - Game.stageGrid.tile.floorHeight)
					});
			}
		}
	}
			...
			if(tileComponent !== ' '){
				Crafty.e(tileComponent)
					.attr({
						x:col * Game.stageGrid.tile.width + paddingXToCenter,
						y:row * (Game.stageGrid.tile.height - Game.stageGrid.tile.floorHeight) + paddingYToCenter,
						z:row * (Game.stageGrid.tile.height - Game.stageGrid.tile.floorHeight) + 1
					});
			}
		}
	}
